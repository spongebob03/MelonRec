...
from dataLoad.arena_util2 import write_json, load_json, remove_seen, most_popular
...
def Recommender(train, questions, n_msp, n_mtp, mode, sim_measure, song_meta, freq_song, save=False):
    ...
    ## 2. 함수 정의
    # 1) Counter 객체에서 빈도수 기준 topk만큼 출력
    ...
    def most_similar_emb(q_id, topk, title=False, genre=False):
        if title:
            plysts = [t[0] for t in title_scores[q_id][:topk]]
            scores = [t[1] for t in title_scores[q_id][:topk]]
        elif genre:
            plysts = [t[0] for t in gnr_scores[q_id][:topk]]
            scores = [t[1] for t in gnr_scores[q_id][:topk]]
        else:
            plysts = [t[0] for t in sim_scores[q_id][:topk]]
            scores = [t[1] for t in sim_scores[q_id][:topk]]
        return plysts, scores
